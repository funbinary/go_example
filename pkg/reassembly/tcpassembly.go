...
type ScatterGather interface {
	...
	Info() (direction TCPFlowDirection, end bool, skip int)
...
func (rl *reassemblyObject) Info() (TCPFlowDirection, bool, int) {
	return rl.Direction, rl.all[len(rl.all)-1].isEnd(), rl.Skip
}
...
func (a *Assembler) AssembleWithContext(netFlow gopacket.Flow, t *layers.TCP, ac AssemblerContext) {
	...
	if half.nextSeq < rev.ackSeq {
		log.Debugf("Delay detected on %v, data is acked but not assembled yet (acked %v, nextSeq %v)", key, rev.ackSeq, half.nextSeq)
	}

	if !half.stream.Accept(t, ci, half.dir, half.nextSeq, &a.start, ac) {
		log.Debugf("Ignoring packet")
		return
	}
	if half.closed {
		// this way is closed
		log.Debugf("%v got packet on closed half", key)
		return
	}
	...
	if half.nextSeq == invalidSequence {
		if t.SYN {
			log.Debugf("%v saw first SYN packet, returning immediately, seq=%v", key, seq)
			seq = seq.Add(1)
		} else if a.start {
			log.Debugf("%v start forced", key)

		} else {
			log.Debugf("%v waiting for start, storing into connection", key)
		}
		half.nextSeq = seq
		action.queue = false
	} else {
		...
		if diff > 0 {
			log.Debugf("%v gap in sequence numbers (%v, %v) diff %v, storing into connection", key, half.nextSeq, seq, diff)

		} else {
			log.Debugf("%v found contiguous data (%v, %v), returning immediately: len:%d", key, seq, half.nextSeq, len(bytes))

			action.queue = false
		}
	}

	action = a.handleBytes(bytes, seq, half, t.FIN, action, ac)
	if len(a.ret) > 0 {
	...
	log.Debugf("%v nextSeq:%d", key, half.nextSeq)
...
func (a *Assembler) checkOverlap(half *halfconnection, queue bool, ac AssemblerContext) {
	...
	if len(bytes) > 0 && queue {
		...
		half.pages += numPages
		if cur != nil {
			log.Debugf("adding %s after %s", p, cur)

			cur.next = p
			p.prev = cur
		} else {
			log.Debugf("adding %s as first", p)

			half.first = p
		}
		if next != nil {
			log.Debugf("setting %s as next of new %s", next, p2)

			p2.next = next
			next.prev = p2
		} else {
			log.Debugf("setting %s as last", p2)
...
func (a *Assembler) dump(text string, half *halfconnection) {
	...
	if half != nil {
		p := half.first
		if p == nil {
			log.Debugf(" * half.first = %p, no chunks queued\n", p)
		} else {
			s := 0
			nb := 0
			log.Debugf(" * half.first = %p, queued chunks:", p)
			for p != nil {
				log.Debugf("\t%s bytes:%s\n", p, hex.EncodeToString(p.bytes))
				s += len(p.bytes)
				nb++
				p = p.next
			}
			log.Debugf("\t%d chunks for %d bytes", nb, s)
		}
		log.Debugf(" * half.last = %p\n", half.last)
		log.Debugf(" * half.saved = %p\n", half.saved)
		p = half.saved
		for p != nil {
			log.Debugf("\tseq:%d %s bytes:%s\n", p.getSeq(), p, hex.EncodeToString(p.bytes))
			p = p.next
		}
	}
	log.Debugf(" * a.ret\n")
...
func (a *Assembler) overlapExisting(half *halfconnection, start, end Sequence, bytes []byte) ([]byte, Sequence) {
	...
	if e != 0 {
		log.Debugf("Overlap detected: ignoring current packet's first %d bytes", diff)
...
func (a *Assembler) handleBytes(bytes []byte, seq Sequence, half *halfconnection, end bool, action assemblerAction, ac AssemblerContext) assemblerAction {
	a.cacheLP.bytes = bytes
	...
	//		log.Debugf("hit max buffer size: %+v, %v, %v", a.AssemblerOptions, half.pages, a.pc.used)
	//
	//		action.queue = false
...
func (a *Assembler) cleanSG(half *halfconnection, ac AssemblerContext) {
	...
	log.Debugf("Remaining %d chunks in SG\n", nbKept)
	log.Debugf("%s\n", a.Dump())
...
func (a *Assembler) sendToConnection(conn *connection, half *halfconnection, ac AssemblerContext) Sequence {
	...
	log.Debugf("after sendToConnection: nextSeq: %d\n", nextSeq)
...
func (a *Assembler) addPending(half *halfconnection, firstSeq Sequence) int {
	...
	for p := half.saved; p != nil; p = p.next {
		log.Debugf("adding pending @%p %s (%s)\n", p, p, hex.EncodeToString(p.bytes))
		ret = append(ret, p)
		s += len(p.bytes)
	}
...
func (a *Assembler) addContiguous(half *halfconnection, lastSeq Sequence) Sequence {
	page := half.first
	if page == nil {
		log.Debugf("addContiguous(%d): no pages\n", lastSeq)
	...
	for page != nil && lastSeq.Difference(page.seq) == 0 {
		log.Debugf("addContiguous: lastSeq: %d, first.seq=%d, page.seq=%d\n", half.nextSeq, half.first.seq, page.seq)
